...
//		 new ClassPathXmlApplicationContext("/resources/application.xml");
	...
	private void allJavaAndFtl(Class<?>[] carray) {
		for (Class<?> c : carray) {
			allJava(c);
			allFtl(c);
		}
	...
	private void allFtl(Class<?> className) {
		...
		if (obj != null) {
			...
			FreemarkerUtil.getInstance(freemarkerConfig).htmlFile(
					root,
					TestCode.class.getResource("").getPath()+"Page_xx_list.ftl",
					SAVEPATH + "/WEB-INF/ftl/admin/" + obj.getModuleName().toLowerCase() + "/" + obj.getClassName().toLowerCase()
							+ "_list.ftl");
			FreemarkerUtil.getInstance(freemarkerConfig).htmlFile(
					root,
					TestCode.class.getResource("").getPath()+"Page_xx_modify.ftl",
					SAVEPATH + "/WEB-INF/ftl/admin/" + obj.getModuleName().toLowerCase() + "/" + obj.getClassName().toLowerCase()
							+ "_modify.ftl");
		}
	}
